...
export default function App(){
  ...
  const callClaude=async history=>{
    const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","anthropic-dangerous-direct-browser-access":"true","x-api-key":import.meta.env.VITE_ANTHROPIC_API_KEY||""},body:JSON.stringify({model:"claude-sonnet-4-5-20251001",max_tokens:3000,system:systemPromptRef.current,messages:history.map(m=>({role:m.role,content:m.content}))})});
