...
export default function App() {
  ...
  const callClaude=async history=>{
    const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","anthropic-dangerous-direct-browser-access":"true","x-api-key":import.meta.env.VITE_ANTHROPIC_API_KEY||""},body:JSON.stringify({model:"claude-sonnet-4-5-20251001",max_tokens:2000,system:systemPrompt,messages:history.map(m=>({role:m.role,content:m.content}))})});
  ...
  const handleOptIn=async(name,email)=>{
    ...
    try{
      await fetch("WEBHOOK_URL",{method:"POST",headers:{"Content-Type":"application/json","anthropic-dangerous-direct-browser-access":"true","x-api-key":import.meta.env.VITE_ANTHROPIC_API_KEY||""},body:JSON.stringify({firstName:name,email,automationName:pendingData.automationName,goal:pendingData.goal,planRequired:pendingData.planRequired,source:"kajabi-automation-builder",shareUrl:getShareUrl(pendingData)})});
    }catch(e){}
  };
